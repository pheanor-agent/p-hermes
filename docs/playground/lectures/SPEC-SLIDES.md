...
> **마지막 업데이트**: 2026-06-29
> **상태**: Active
> **연결**: TEMPLATE.md (playground), docs/playground/lectures/

---

## 1. 목적
p-hermes 강의 슬라이드의 **playground (개발) 버전**을 정의합니다.
이 Spec은 로컬 개발 공간인 `docs/playground/lectures/`의 슬라이드에 적용됩니다.

## 2. 버전 관리
- **v2.1**: JOB-1962 슬라이드 오버플로우 수정 (box-sizing, padding 48px), JOB-1963 사실 정합성 수정 (Skill 구조, 실패 순서, Runtime 설명)
- **v1.9-playground**: §10 nav JS currentSlide 위치 규칙, §14 hero CSS 공유 강제, §17 키보드 내비게이션 신규 (JOB-1868)
...
## 3. 스펙 참조
- TEMPLATE.md: `docs/playground/lectures/TEMPLATE.md` — 레이아웃/스타일 규칙
...
| 강의 | 버전 | 상태 | Spec 연동 |
|:----|:----:|:----:|:---------|
| 01 — Why Agents Fail | v1.9-playground | ✅ courses→lectures, 장수 업데이트 (JOB-1867) | SPEC-SLIDES v1.9 |
| 02 — Memory & Knowledge | v1.9-playground | ✅ courses→lectures, 장수 업데이트 (JOB-1867) | SPEC-SLIDES v1.9 |
| 03 — Skills & Workflow | v1.9-playground | ✅ courses→lectures, 장수 업데이트 (JOB-1867) | SPEC-SLIDES v1.9 |
| 04 — Hermes Core Architecture | v1.9-playground | ✅ courses→lectures, 장수 업데이트 (JOB-1867) | SPEC-SLIDES v1.9 |
| 05 — Knowledge Factory | v1.9-playground | 🚧 Template Only | SPEC-SLIDES v1.9 |
| 06 — Content Factory | v1.9-playground | 🚧 Template Only | SPEC-SLIDES v1.9 |
| 07 — Research Factory | v1.9-playground | 🚧 Not Started | SPEC-SLIDES v1.9 |
| 08 — Spec-Driven Development | v1.9-playground | 🚧 Not Started | SPEC-SLIDES v1.9 |

## 8. 게이트
- `docs/playground/lectures/`의 슬라이드는 검증 실패해도 진행 가능 (WIP 특성)
...
if (currentSlide && hideTypes.some(t => currentSlide.classList.contains(t))) {
...
> ⚠️ **critical**: `currentSlide` 변수를 `hideTypes.some()` 호출 **전에** 반드시 정의해야 함 (JOB-1868 L04 JS bug 교훈)
...
- **템플릿 관련 CSS(.hero, .cover, .ring 등)는 공유 CSS에만 정의** (MUST) — 로컬 재정의 금지

### 14.3 공유 CSS 필수 포함 (§14.2 적용)
공유 CSS(`slides-components.css`)에는 다음이 **반드시** 포함된다 (MUST):
- `.cover-hero` 컨테이너 스타일
- `.hero .ring`, `.hero .ring::before`, `.hero .ring-text` (Cover 원형)
- `.hero-slide .title`, `.hero-slide .subtitle`
- `.lecture-badge`
- `.goal-section`, `.goal-tag`, `.goal-why`, `.goal-item`, `.goal-flow`
- `.question`, `.insight-box`

### 14.4 파일 위치
```
docs/playground/lectures/
├── components/
...
├── lecture-05-knowledge-factory.html
├── lecture-06-content-factory.html
└── SPEC-SLIDES.md
```
...
- **`e.preventDefault()` 필수** (MUST): 화살표 키의 browser default scroll 동작 차단
- **`scrollTo()` 사용** (MUST): `scrollBy({behavior: smooth})` 대신 `scrollTo({left: target})`
- **`scroll-snap` 충돌 주의**: `scroll-snap-type: x mandatory`와 smooth scroll은 충돌 가능
...
document.addEventListener('keydown', (e) => {
  ...
  if (isArrow) e.preventDefault(); // Browser default scroll 차단
  
  if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
    const target = deck.scrollLeft + vw;
