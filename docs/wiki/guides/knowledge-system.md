...
이를 해결하기 위해 p-hermes는 **'신호 감지 $\\rightarrow$ 임시 저장 $\\rightarrow$ 정제 $\\rightarrow$ 분류'**라는 엄격한 파이프라인을 적용합니다.
...
- **`build-scores.sh`**: 각 지식 페이지의 참조 횟수와 최신성을 계산하여 T1, T2, T3 등급을 부여합니다. 이는 지식의 '가치'를 정량적으로 평가하는 시스템입니다.
    - **T1 (Hot)**: 매우 자주 참조되는 핵심 지식 (예: 프로젝트 핵심 명세) $\\rightarrow$ 최상단 인덱스에 배치.
    - **T2 (Warm)**: 간혹 참조되는 지식 (예: 특정 라이브러리 사용법).
    - **T3 (Cold)**: 오랫동안 참조되지 않은 지식 $\\rightarrow$ 정리 및 아카이빙 대상.
