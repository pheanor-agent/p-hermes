...
p-hermes의 지식 시스템은 에이전트에게 **'영구적인 장기 기억(Long-term Memory)'**을 부여하는 장치입니다. 비유하자면, 에이전트가 모든 대화를 마친 후 중요한 내용만 요약해서 **'개인 위키(Personal Wiki)'**에 기록하고, 나중에 비슷한 문제가 발생했을 때 그 위키를 검색해서 \"아, 예전에 이렇게 해결했었지!\"라고 회상하는 것과 같습니다. 단순히 로그를 쌓아두는 것이 아니라, '정제된 지식'을 관리하는 것이 핵심입니다.
...
p-hermes는 **'수집 $\rightarrow$ 가공 $\rightarrow$ 저장'**이라는 정교한 파이프라인을 통해 노이즈를 제거하고 순수 지식만을 추출하여 저장합니다.
...
에이전트는 단순히 키워드 매칭을 하는 것이 아니라, 다음과 같은 단계적 순서로 지식을 탐색합니다:
1. **Full-Text Search (FTS)**: SQLite 기반의 고속 검색을 통해 키워드가 포함된 모든 문서를 1차 필터링하여 후보군을 뽑습니다.
2. **Domain Navigation**: `index.md` $\rightarrow$ `도메인 폴더` $\rightarrow$ `상세 문서` 순으로 계층적으로 접근하여, 해당 지식이 어떤 맥락(Context)에서 작성되었는지 파악합니다.
3. **Source Trace (근거 추적)**: 위키에 기록된 정보가 의심스럽거나 더 자세한 맥락이 필요할 경우, 위키에 링크된 원본 세션 기록(`runtime/workspace/jobs/JOB-XXXX/`)으로 돌아가 실제 대화 내용을 확인합니다.
