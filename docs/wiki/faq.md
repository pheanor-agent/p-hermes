...
`[JOB-XXXX]` 태그는 단순한 식별자가 아니라 Hermes의 **'모드 전환 스위치'**입니다. 이 태그를 사용하면 Hermes는 단순 채팅 모드에서 **'공식 프로젝트 모드'**로 전환되어 다음과 같은 전문 공정을 수행합니다:
- **상태 추적**: 9단계 워크플로우(Investigation $\rightarrow$ Design $\rightarrow$ ... $\rightarrow$ Done)를 엄격히 따릅니다.
- **물리적 기록**: 모든 과정이 `~/.hermes/runtime/workspace/jobs/JOB-XXXX/` 폴더에 파일로 남습니다.
- **검증 강제**: `Test` 단계를 통과하지 못하면 작업을 종료하지 않고 스스로 수정합니다.

반면, 간단한 작업(예: "이 문장을 영어로 번역해줘", "오늘 날씨 어때?")은 `[TASK]` 태그를 쓰거나 일반 채팅으로 요청하는 것이 훨씬 빠르고 효율적입니다.
...
### Q5. 작업 도중에 모델을 직접 바꿀 수 있나요? (예: GPT-4 $\rightarrow$ Claude)
...
`[JOB-XXXX]` 형식을 통해 진행된 작업의 모든 흔적은 `~/.hermes/runtime/workspace/jobs/JOB-XXXX/` 폴더에 저장됩니다.
