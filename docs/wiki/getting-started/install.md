...
`git clone` 후 `bash setup.sh` 한 줄로 Hermes Agent의 완전한 실행 환경을 구축합니다.
...
새로운 머신이나 가상 환경에서 Hermes를 구축할 때 수동으로 디렉토리를 생성하고 설정 파일을 복사하면 누락과 불일치가 발생하기 쉽습니다. API 키를 직접 설정 파일에 적으면 보안 리스크가 있으며, WSL과 macOS 간 환경 차이로 인한 경로 오류가 빈번합니다.
