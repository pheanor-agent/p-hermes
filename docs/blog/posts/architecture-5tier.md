...
domain: architecture
type: blog
title: "5계층 아키텍처: 왜 모듈화가 에이전트 시스템의 생존 조건인가"
date: 2026-06-17
version: "1.0.0"
compatibility: v0.16.0
author: p-hermes
status: published
tags: ["architecture", "5-tier", "modularization", "event-driven", "agent-system", "async-communication", "audit-trail"]
related_specs: ["SPEC-D04"]
---

# 5계층 아키텍처: 왜 모듈화가 에이전트 시스템의 생존 조건인가

> **💡 한 줄 요약**: Hermes의 5-Tier 아키텍처는 Core, Config, Workflow, Script, Release로 수직 분리를 통해 수평 확장을 가능하게 하며, event.sh 단일 진입점과 JSONL 감사 추적을 통해 시스템 신뢰성을 확보합니다.

---

## 💡 한 줄 요약

Hermes의 5-Tier 아키텍처는 수직 분리로 수평 확장을 가능하게 하며, event.sh 단일 진입점과 JSONL 감사 추적을 통해 시스템 신뢰성을 확보합니다.
...
Hermes는 이 문제를 구조적 분리로 해결합니다. **5계층 아키텍처** — Core, Config, Workflow, Script, Release — 는 각 계층이 단일 책임을 가지도록 강제합니다. 설계 철학은 명확합니다: **'수직 분리가 수평 확장을 가능하게 한다'**.
...
## 🔍 문제 상황: 스크립트 간 직접 호출의 위험과 교착 상태

### 단일 진입점 부재의 문제

5계층 아키텍처를 도입하기 전, 여러 AI 에이전트 프레임워크는 스크립트 간 직접 호출 패턴을 사용했습니다. 각 기능 스크립트가 다른 기능을 호출할 때 직접 `source` 또는 `bash` 명령으로 실행하는 방식입니다.

```.bash
# 문제 사례: 스크립트 간 직접 호출 (직접 의존성)
# setup.sh 가 backup.sh를 직접 호출
# backup.sh 가 deploy.sh를 직접 호출
# deploy.sh 가 notify.sh를 직접 호출
# → A → B → C → D 가 모두 서로의 존재를 '알고' 있음
source backup.sh
source deploy.sh
source notify.sh
```

이 패턴이 초래하는 문제는 세 가지로 집약됩니다.

### 1. 교착 상태 (Deadlock) 발생 가능성

스크립트 A가 B를 호출하고, B가 A가 자원을 잠궈둔 상태로 대기하면, 두 프로세스는 서로를 기다리며 영원히 멈춥니다. 실제로 CI/CD 파이프라인에서 이런 교착 상태가 빈번하게 발생합니다.

**실제 사례**: Hermes 초기 빌드 파이프라인에서 `validate-links.sh`와 `deploy.sh` 간 교착 상태가 발생했습니다. `validate-links.sh`는 `deploy.sh`가 생성한 링크 인덱스를 읽고, `deploy.sh`는 `validate-links.sh`가 완료 신호를 남길 때까지 대기했습니다. 두 스크립트가 직접 호출 관계에 있었기 때문에, 어느 쪽이 먼저 깨어나지 못하는 교착 상태에 빠졌습니다.

**해결**: event.sh를 통한 비동기 통신으로 변경하여, 두 스크립트가 상태 파일을 읽고 쓰는 방식으로 통신하게 했습니다.

```
# 교착 상태 다이어그램
# A ──(기다림)──→ B ──(기다림)──→ A  → 교착
# A ──(event)──→ 상태파일 ←──(event)── B  → 해결
```

### 2. 실패 전파 (Failure Propagation)

직접 호출 체인에서 중간 단계가 실패하면, 호출자 측에서 복구 기회를 갖지 못한 채 전체 파이프라인이 중단됩니다. 상위 스크립트가 하위 스크립트의 실패를 캐치하고 대체 경로를 선택하려면, 하위 스크립트의 실패 케이스를 모두 파악해야 합니다. 이는 결합도를 더욱 높이는 악순환을 만듭니다.

### 3. 의존성 순환 (Circular Dependency)

직접 호출이 허용되면, A가 B를, B가 C를, C가 A를 호출하는 순환 의존성이 자연스럽게 발생하기 쉽습니다. 순환 의존성은 빌드 순서를 정의할 수 없게 만들며, 수정 시 전체 호출 그래프를 이해해야 하는 부담을 초래합니다.

**실제 사고**: Hermes Cron 시스템 초기 구현에서 `cron-wrapper.sh`가 `workflow-gate.sh`를 호출하고, `workflow-gate.sh`가 `cron-wrapper.sh`의 상태 파일을 읽는 순환 의존성이 발생했습니다. Cron Job이 중복 실행되거나, 상태 전이가 예상과 다른 순서로 발생하는 원인이 되었습니다.
...
## 📊 5-Tier 분리: 수직 분리가 수평 확장을 가능하게 하는 메커니즘

### 계층 간 의존성 방향

5계층 아키텍처의 핵심 규칙은 **하위 계층에서 상위 계층으로만 의존**할 수 있다는 점입니다. Core(T1)는 어떤 계층도 의존하지 않습니다. Release(T5)는 모든 하위 계층을 사용할 수 있습니다.
...
5계층 아키텍처가 제공하는 확장 메커니즘은 다음과 같습니다.
...
## 💡 활용 예시: 5계층 아키텍처 하에서의 Cron Job 추가

새로운 Cron Job을 Hermes 시스템에 추가하는 과정을 통해 5계층 아키텍처의 작동 방식을 확인합니다.
...
- [왜 9단계 상태머신인가?](./why-9-step-workflow.md): 5계층 아키텍처 위에서 작동하는 상태 머신 설계.
...
Hermes의 5계층 아키텍처는 마이크로서비스 아키텍처(MSA) 설계 원칙과 동일한 철학을 공유합니다. 마이크로서비스가 애플리케이션을 서비스 경계로 나누고 REST/gRPC로 통신하는 것처럼, Hermes는 스크립트를 계층 경계로 나누고 event.sh + 상태 파일로 통신합니다.
...
**독립적 배포**: 마이크로서비스의 핵심 이점 중 하나는 개별 서비스의 독립적 배포입니다. Hermes의 5계층 구조는 Script 계층의 변경이 다른 계층에 영향을 주지 않으므로, 동일한 독립적 배포를 스크립트 레벨에서 실현합니다.
...
**3. 다중 에이전트 오케스트레이션**: 5계층 아키텍처는 다중 에이전트 환경에 자연스럽게 확장됩니다. 각 에이전트가 독립적인 T4 Script 계층을 가지며, 공통의 event.sh를 통해 이벤트 기반 통신을 수행합니다. 이벤트 필터링과 라우팅 규칙을 통해 에이전트 간 협력과 경쟁을 조정합니다.

### 모듈화 플랫폼의 궁극적 목표

모듈화 플랫폼의 궁극적 목표는 **'투명한 확장'**입니다. 새로운 기능을 추가할 때 기존 코드를 그대로 유지하며, 새로운 모듈만 추가하면 시스템이 자동으로 통합합니다. 5계층 아키텍처는 이 목표를 향한 구조적 기반입니다.

Hermes는 현재 5계층 아키텍처를 파일 시스템과 쉘 스크립트 레벨에서 구현하고 있습니다. 마이크로서비스 아키텍처의 설계 원칙을 지속적으로 적용하며, 플랫폼의 확장성과 신뢰성을 높여갑니다.

---

_모듈화는 에이전트 시스템의 생존 조건입니다. 단단하게 결합된 시스템은 작은 변화에도 무너지지만, 느슨하게 결합된 시스템은 새로운 기능의 추가에 견고하게 대응합니다. Hermes의 5계층 아키텍처는 이 원리를 에이전트 시스템에 구현한 사례입니다._
