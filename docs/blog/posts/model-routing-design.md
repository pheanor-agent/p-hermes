...
$ cat ~/.hermes/workspace/jobs/JOB-1550/cost-report.json
{
  "job_id": "JOB-1550",
  "model_used": "단일 모델",
  "steps": [
...
$ cat ~/.hermes/workspace/jobs/JOB-1588/cost-report.json
{
  "job_id": "JOB-1588",
  "steps": [
...
### 1. 모델 역할 정의 (`config.yaml` + `model-roles.yaml`)

시스템 설정 파일(config.yaml)과 역할 정의 파일(model-roles.yaml)의 이중 구조로 라우팅을 제어합니다. config.yaml에서 기본 모델과 공급자를 지정하고, model-roles.yaml에서 역할별 구체적인 매핑을 정의합니다.

**역할 체계**: `default`(기본) → `creative`(창작) → `coding`(코딩) → `reasoning`(추론) → `fast`(빠른 응답) → `review`(검증)의 6개 역할을 지원합니다.

```yaml
# config.yaml — 기본 설정
model:
  default: fast          # 기본 역할
  provider: custom       # 기본 공급자

# model-roles.yaml — 역할별 상세 매핑 (SSOT)
roles:
  default:
    model: fast
    provider: custom
...
| 역할 (Role) | 핵심 요구 역량 | 주요 작업 |
| :--- | :--- | :--- |
| **fast** | 빠른 응답, 폭넓은 검색 | 파일 구조 분석, 시스템 현황 파악 (Investigation) |
| **reasoning** | 논리적 추론, 창의적 설계 | 아키텍처 설계, `design.md` 작성 |
| **review** | 세밀한 검증, 오류 탐지 | 설계서 검토, 코드 리뷰, `review.md` 작성 |
| **coding** | 구문 정확성, 구현 속도 | 실제 파일 수정, 스크립트 구현 |
...
Hermes의 모델 라우팅은 `model-roles.yaml`을 SSOT(Single Source of Truth)로 사용합니다. 작업 단계(Step)별로 필요한 역할을 `workflow.sh`가 판단하고, 해당 역할에 매핑된 모델을 `model-roles.yaml`에서 조회하여 호출합니다. 각 단계의 산출물(design.md, review.md 등)에 메타데이터로 사용된 역할을 기록하여 추적성을 확보합니다.

```bash
# 라우팅 동작 예시 — 작업 단계별 역할 사용
# Investigation 단계 → fast 역할
# Design 단계 → reasoning 역할
# Review 단계 → review 역할
# Execution 단계 → coding 역할
```
...
```bash
$ cat ~/.hermes/workspace/jobs/model-token-distribution.json
{
...
- **역할 기반 라우팅 적용 시**:
    - 비용: $40 / 작업당 (**66% 절감**)
    - 시간: 30분 (**33% 단축**)
    - 오류: Review 단계에서 Design 단계의 논리적 허점을 미리 발견 → Execution 단계 0회 롤백.
...
어떤 모델이 어떤 역할에서 실제 성능이 좋은지 추적하려면 데이터가 필요합니다. Hermes는 각 라우팅 호출의 결과를 기록하여 모델 성능을 지속적으로 모니터링합니다. 로그 데이터와 JOB 산출물 분석을 통해 역할별 최적 모델을 주기적으로 재평가합니다.

이 데이터를 기반으로 각 역할에 가장 적합한 모델을 지속적으로 최적화합니다.
