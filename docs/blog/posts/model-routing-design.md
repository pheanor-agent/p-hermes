...
```
평균 지능:  Claude > Gemma > Qwen > GLM
코드 생성:  Qwen > Gemma > Claude > GLM
논리 추론:  Gemma > Claude > Qwen > GLM
빠른 응답:  GLM > Qwen > Gemma > Claude
비용 효율:  GLM > Qwen > Gemma > Claude
```
...
同一 모델이 설계도 하고 검증도 하면, 자신의 논리적 오류를 감지하지 못하는 경향이 있습니다. 이는 AI 모델이 '자기 생성한 텍스트'를 평가할 때 보이는 고유한 현상입니다.
...
$ cat ~/.hermes/runtime/state/jobs/JOB-1588/cost-report.json
{
  "job_id": "JOB-1588",
  "steps": [
    {"step": "investigation", "model": "glm-4", "cost": 1.20, "tokens": 28000, "time_min": 3},
    {"step": "design", "model": "gemma-4", "cost": 12.00, "tokens": 85000, "time_min": 10},
    {"step": "review", "model": "claude-3-5-sonnet", "cost": 15.00, "tokens": 56000, "time_min": 8},
    {"step": "execution", "model": "qwen-2.5", "cost": 5.50, "tokens": 72000, "time_min": 7},
    {"step": "test", "model": "glm-4", "cost": 1.80, "tokens": 35000, "time_min": 2}
  ],
  "total_cost": 35.50,
  "total_time_min": 30,
  "rollbacks": 0
...
```bash
# Design(Gemma-4)과 Review(Claude 3.5)의 서로 다른 관점
$ cat jobs/JOB-1588/review.md

[REVIEW: Claude 3.5 Sonnet]
Design(Gemma-4)이 제안한 계획에서 2개의 결함을 발견:

1. [중요] backup.sh 수정 누락
   - Gemma는 core/scripts/ 폴더의 스크립트만 수정 계획
   - 하지만 infra/backups/ 폴더의 백업 스크립트도 동일 경로 참조
   - 해결: design.md에 backup.sh 수정 Phase 추가 요청

2. [중간] 실행 순서 오류
   - Gemma는 config.yaml 수정을 Phase 1로 배치
   - 하지만 config 변경 후 scripts 재구성이 필요한데 순서가 뒤바꿈
   - 해결: Phase 순서 재배치 요청
```

Design을 한 모델(Gemma)이 아닌 다른 모델(Claude)이 검증했기 때문에 Gemma의 '생각 패턴'과는 다른 관점에서 결함을 발견했습니다.
...
graph TD
    ...
    Action4 --> Step5[Test]
    Step5 -->|Route: GLM-4| Action5[테스트 실행 및 검증]
...
$ cat ~/.hermes/runtime/state/model-token-distribution.json
{
  ...
  "by_model": {
    "glm-4": {"tokens": 5680000, "percentage": 40.0, "roles": ["investigation", "test"]},
    "gemma-4": {"tokens": 4260000, "percentage": 30.0, "roles": ["design"]},
    "claude-3-5-sonnet": {"tokens": 2840000, "percentage": 20.0, "roles": ["review"]},
    "qwen-2.5": {"tokens": 1420000, "percentage": 10.0, "roles": ["execution"]}
  },
...
GLM-4가 40%의 토큰을 처리하지만 비용은 전체의 2.8%만 차지합니다. Claude 3.5는 20%의 토큰만 처리하지만 비용의 62%를 차지합니다. 역할 기반 라우팅은 고가 모델을 '검증'에만 집중함으로써 비용 효율을 극대화합니다.
...
Claude API가 다운되었을 때 Review를 Gemma-4로 대체하여 계속 운영 가능합니다. 품질이 다소 떨어지지만, 작업이 완전히 중단되는 것을 방지합니다.
...
$ cat ~/.hermes/runtime/state/model-performance.json
{
  ...
  "design_role": {
    "gemma-4": {"jobs": 23, "review_pass_rate": 0.78, "avg_time_min": 10},
    "claude-3-5": {"jobs": 3, "review_pass_rate": 0.83, "avg_time_min": 14}
  },
  "execution_role": {
    "qwen-2.5": {"jobs": 28, "syntax_error_rate": 0.03, "avg_time_min": 7},
    "gemma-4": {"jobs": 5, "syntax_error_rate": 0.06, "avg_time_min": 9}
  }
}
```

이 데이터를 기반으로 Gemma-4가 Design 역할에서 비용 대비 성능이 가장 좋으며, Qwen-2.5가 Execution에서 구문 오류률이 가장 낮음을 확인했습니다.
