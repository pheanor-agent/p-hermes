...
스크립트, 설정 파일, 로그가 한데 섞여 있다 보니, `A 스크립트` → `B 설정 읽기` → `B가 A를 호출`하는 순환 참조가 발생했습니다.
...
- **과거**: `cp -r ~/.hermes /new-server` → 50GB 복사 → 45분 소요 → 설정 충돌 발생.
- **현재**: `cp -r core/ release/ /new-server` → 100MB 복사 → **5분 소요** → 즉시 실행 가능.
...
- [\"텍스트 규칙 → 스크립트 강제\" 철학](https://pheanor-agent.github.io/p-hermes/docs/blog/posts/structural-enforcement.md): 5-Tier 구조를 유지하기 위한 `check-symlink.sh` 등 강제 도구.

---

_5-Tier 구조는 데이터의 생명주기와 성격을 정의하는 아키텍처입니다. 공간을 분리함으로써 우리는 시스템의 예측 가능성을 얻었습니다._
