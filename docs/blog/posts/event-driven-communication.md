...
초기 Hermes는 단순한 쉘 스크립트 연쇄 호출 방식을 사용했습니다. `지식 업데이트` → `백업 실행` → `세션 정리` 순으로 직접 실행되는 구조였습니다.
...
- **사례**: `지식 업데이트` 도중 디스크 용량 부족으로 에러 발생 → 이어서 실행되던 `백업 스크립트`가 불완전한 데이터를 백업 → 결과적으로 원본과 백업본이 동시에 파괴됨.

### 3. 동시성 충돌 (Concurrency Clash)
두 개의 에이전트가 동시에 하나의 관리 스크립트를 호출할 때 데이터가 엉키는 현상입니다.
- **사례**: Hermes와 OpenClaw가 동시에 `backup.sh`를 호출 → 서로 같은 백업 파일에 쓰기를 시도 → 파일 락(Lock) 충돌로 인해 백업 파일이 깨짐.
...
Hermes는 **\"함수 호출\"을 \"파일 변경 이벤트\"로 대체**했습니다. 이제 도메인 간의 통신은 `상태 파일 작성` → `비동기 감지` → `처리` 순으로 이루어집니다.
...
- **기존**: `업데이트 실패` → `백업 실패` → `전체 중단` (복구 시간 4시간)
- **현재**:
