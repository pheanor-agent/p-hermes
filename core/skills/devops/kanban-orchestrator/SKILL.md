...
## Worker Pitfalls and Patterns (absorbed from kanban-worker)

> The core worker lifecycle (orient → work → heartbeat → block/complete) is auto-injected via `KANBAN_GUIDANCE`. These are the edge cases and good patterns.
...
| `worktree` | Git worktree | If `.git` missing, run `git worktree add <path>` first. |
...
Shape `metadata` so downstream parsers can use it:
...
# Review task needing human review:
...
Only list `created_cards` with IDs you captured from successful `kanban_create` returns — never invent ids.
...
Name progress: `"epoch 12/50, loss 0.31"`, `"scanned 1.2M/2.4M rows"`. Skip for tasks under ~2 min.
