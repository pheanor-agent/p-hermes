...
### ⚠️ Deck Pitfalls (JOB-1503 학습)

**슬라이드 설계 vs 실제 파일 불일치**: 사용자가 정의한 슬라이드 구성이 실제 HTML 파일과 다르면 즉시 지적받음. 작성 후 반드시 슬라이드 수와 구성 확인 필수. 슬라이드당 `<div class="slide">` 요소 수를 실제 슬라이드 수와 비교.

**실행 후 브라우저 테스트 강제**:
- HTML 파일 작성 후 **반드시 브라우저로 검증** (`browser_navigate`로 URL 확인)
- GitHub Pages 배포 시 **1분 대기 후 확인** (CDN 캐시로 즉시 반영 안 됨)
- 캐시 무효화: 쿼리 파라미터 `?t=$(date +%s)` 추가
- 테스트 없이 "완료" 선언 시 사용자 지적 받음
- 상세 프로세스: `references/github-pages-deployment.md` 참조

**GitHub Pages gh-pages 브랜치 푸시**:
- `git checkout --orphan gh-pages-temp` 실패 시 기존 브랜치 존재 확인 (`git branch -a | grep gh-pages`)
- 기존 브랜치 존재 시 `git branch -D gh-pages-temp` 후 다시 생성
- `git rm -rf .` 후 파일 복사 → `git add` → `git commit` → `git push origin gh-pages --force`
- main 브랜치에 잘못 커밋되지 않도록 주의 (orphan 브랜치에서 작업 완료 후 `git checkout main`)
- **브랜치 오염 방지**: `git reset --hard <correct-commit>`으로 main 브랜치 복원

**파일 검증 누락**: `docs/slides.html` 등 산출물 파일 존재 확인 후 JOB 완료. `ls -la docs/`로 파일 존재 확인 필수. 파일 삭제됨 시 `git checkout main -- docs/slides.html`로 복원.

**슬라이드당 텍스트 양**: 200-400자 제한 초과 시 가독성 저하. `overflow: hidden` + `100vh` + flexbox 중앙 정렬 조합으로 화면 고정.

**선택적 기술 내용 노출 (팝업 패턴)**:
- 핵심 개념은 메인 슬라이드에, 기술적 상세는 팝업/모달로 분리
- `position: fixed; z-index: 200;` 오버레이 + `Escape`/클릭 시 닫힘
- JavaScript `showMore()`/`hideMore()` 함수로 토글
- 기술적 내용 JSON 객체에 저장 후 동적 렌더링
- 예: 내부 시스템 수치 (3,651 페이지 등)는 팝업으로 선택적 노출

**워크플로우 준수**:
- 테스트/리뷰 단계 생략 금지 (JOB-1503 교훈)
- 설계 → 실행 → **테스트** → 리뷰 → 완료 순서 강제
- 중간 변경 사항도 `execution.md`에 기록
