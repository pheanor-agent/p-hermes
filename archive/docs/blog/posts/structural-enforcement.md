# \"텍스트 규칙 → 스크립트 강제\" 철학: Spec-Driven Development
...
**\\\"더 똑똑한 모델(GPT-4 → Claude 3.5)을 쓴다고 해결될 문제가 아니었습니다. 규칙을 강제할 '물리적 장치'의 부재였습니다.\\\"**
...
**텍스트 규칙**: \"9단계 상태머신(Investigation → Design → ...)을 준수하라.\"
...
- **메커니즘**: `jq`를 이용한 상태 검증 → 유효하지 않은 전이 시 에러 메시지 출력 → 에이전트는 에러를 해결(앞 단계를 완료)해야만 진행 가능.
...
- **이전의 대응**: \"심링크 쓰지 마!\"라고 프롬프트를 수정함 → 며칠 뒤 다시 심링크 생성.
